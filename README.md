...
코드 수정으로 라인이 추가/삭제되어 북마크의 `lineNumber`가 어긋났을 때,
각 북마크에 저장된 `lineText`를 실제 파일에서 재탐색해 올바른 라인으로 복구합니다.
`lineText`만 보존되면 라인이 밀려도 안전하게 위치를 되찾습니다.
...
| 인자 | 설명 | 기본값 |
|---|---|---|
| `폴더명` | 원복할 폴더명 (생략 시 전체) | 전체 |
| `--root` | `.bookmarks.json` 위치 | 자동 탐색 |
| `--dry-run` | 파일 미수정, 이동 미리보기만 출력 | off |

- `MOVED` — 라인이 밀려 새 위치로 복구됨
- `OK` — 이미 올바른 위치
- `LOST` — `lineText`가 코드에서 사라짐 (수동 확인 필요)
- `MISSING` — 파일 자체가 없음
